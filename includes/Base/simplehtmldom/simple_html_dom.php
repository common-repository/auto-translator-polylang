...
define('ATFP_HDOM_TYPE_ELEMENT', 1);
define('ATFP_HDOM_TYPE_COMMENT', 2);
define('ATFP_HDOM_TYPE_TEXT', 3);
define('ATFP_HDOM_TYPE_ENDTAG', 4);
define('ATFP_HDOM_TYPE_ROOT', 5);
define('ATFP_HDOM_TYPE_UNKNOWN', 6);
define('ATFP_HDOM_QUOTE_DOUBLE', 0);
define('ATFP_HDOM_QUOTE_SINGLE', 1);
define('ATFP_HDOM_QUOTE_NO', 3);
define('ATFP_HDOM_INFO_BEGIN', 0);
define('ATFP_HDOM_INFO_END', 1);
define('ATFP_HDOM_INFO_QUOTE', 2);
define('ATFP_HDOM_INFO_SPACE', 3);
define('ATFP_HDOM_INFO_TEXT', 4);
define('ATFP_HDOM_INFO_INNER', 5);
define('ATFP_HDOM_INFO_OUTER', 6);
define('ATFP_HDOM_INFO_ENDSPACE', 7);

defined('ATFP_DEFAULT_TARGET_CHARSET') || define('ATFP_DEFAULT_TARGET_CHARSET', 'UTF-8');
defined('ATFP_DEFAULT_BR_TEXT') || define('ATFP_DEFAULT_BR_TEXT', "\r\n");
defined('ATFP_DEFAULT_SPAN_TEXT') || define('ATFP_DEFAULT_SPAN_TEXT', ' ');
defined('ATFP_MAX_FILE_SIZE') || define('ATFP_MAX_FILE_SIZE', 600000);
define('ATFP_HDOM_SMARTY_AS_TEXT', 1);

function ATFP_file_get_html (
  $url,
  ...
  $target_charset = ATFP_DEFAULT_TARGET_CHARSET,
  $stripRN = true,
  $defaultBRText = ATFP_DEFAULT_BR_TEXT,
  $defaultSpanText = ATFP_DEFAULT_SPAN_TEXT) {
  if ($maxLen <= 0) {
    $maxLen = ATFP_MAX_FILE_SIZE;
  }
...
function ATFP_str_get_html (
  ...
  if (empty($str) || strlen($str) > ATFP_MAX_FILE_SIZE) {
    $dom->clear();
    return false;
  }
...
function ATFP_dump_html_tree ($node, $show_attr = true, $deep = 0) {
  $node->dump($node);
}

class ATFP_simple_html_dom_node {
  public $nodetype = ATFP_HDOM_TYPE_TEXT;
  public $tag = 'text';
  public $attr = [];
  public $children = [];
  public $nodes = [];
  ...
  function dump_node ($echo = true) {
    ...
    if (isset($node->_[ATFP_HDOM_INFO_INNER])) {
      $string .= "'" . $node->_[ATFP_HDOM_INFO_INNER] . "'";
    } else {
      $string .= ' NULL ';
    }
  ...
  function find_ancestor_tag ($tag) {
    global $atfp_debug_object;
    if (is_object($atfp_debug_object)) {
      $atfp_debug_object->debug_log_entry(1);
    }
    ...
    while (!is_null($ancestor)) {
      if (is_object($atfp_debug_object)) {
        $atfp_debug_object->debug_log(2, 'Current tag is: ' . $ancestor->tag);
      }
  ...
  function innertext () {
    if (isset($this->_[ATFP_HDOM_INFO_INNER])) {
      return $this->_[ATFP_HDOM_INFO_INNER];
    }

    if (isset($this->_[ATFP_HDOM_INFO_TEXT])) {
      return $this->dom->restore_noise($this->_[ATFP_HDOM_INFO_TEXT]);
    }
  ...
  function outertext () {
    ...
    if (is_object($atfp_debug_object)) {
      ...
      $atfp_debug_object->debug_log(1, 'Innertext of tag: ' . $this->tag . $text);
    }
    ...
    if (isset($this->_[ATFP_HDOM_INFO_OUTER])) {
      return $this->_[ATFP_HDOM_INFO_OUTER];
    }
    ...
    if ($this->dom && $this->dom->nodes[$this->_[ATFP_HDOM_INFO_BEGIN]]) {
      $ret = $this->dom->nodes[$this->_[ATFP_HDOM_INFO_BEGIN]]->makeup();
    }

    if (isset($this->_[ATFP_HDOM_INFO_INNER])) {
      // todo: <br> should either never have ATFP_HDOM_INFO_INNER or always
      if ($this->tag !== 'br') {
        $ret .= $this->_[ATFP_HDOM_INFO_INNER];
      }
    } elseif ($this->nodes) {
      foreach ($this->nodes as $n) {
    ...
    if (isset($this->_[ATFP_HDOM_INFO_END]) && $this->_[ATFP_HDOM_INFO_END] != 0) {
      $ret .= '</' . $this->tag . '>';
    }
  ...
  function text () {
    ...
    switch ($this->nodetype) {
      case ATFP_HDOM_TYPE_TEXT:
        return $this->dom->restore_noise($this->_[ATFP_HDOM_INFO_TEXT]);
      case ATFP_HDOM_TYPE_COMMENT:
        return '';
      case ATFP_HDOM_TYPE_UNKNOWN:
        return '';
    }
    ...
    // In rare cases, (always node type 1 or ATFP_HDOM_TYPE_ELEMENT - observed
  ...
  function makeup () {
    ...
    foreach ($this->attr as $key => $val) {
      ...
      $ret .= $this->_[ATFP_HDOM_INFO_SPACE][$i][0];

      //no value attr: nowrap, checked selected...
      if ($val === true) {
        $ret .= $key;
      } else {
        switch ($this->_[ATFP_HDOM_INFO_QUOTE][$i]) {
          case ATFP_HDOM_QUOTE_DOUBLE:
            $quote = '"';
            break;
          case ATFP_HDOM_QUOTE_SINGLE:
            $quote = '\'';
            break;
          default:
            $quote = '';
        ...
        $ret .= $key
          . $this->_[ATFP_HDOM_INFO_SPACE][$i][1]
          . '='
          . $this->_[ATFP_HDOM_INFO_SPACE][$i][2]
          . $quote
          . $val
          . $quote;
      }
    ...
    return $ret . $this->_[ATFP_HDOM_INFO_ENDSPACE] . '>';
  }
  ...
  function find ($selector, $idx = null, $lowercase = false) {
    ...
    for ($c = 0; $c < $count; ++$c) {
      ...
      if (!isset($this->_[ATFP_HDOM_INFO_BEGIN])) {
        return [];
      }

      $head = [$this->_[ATFP_HDOM_INFO_BEGIN] => 1];
      $cmd  = ' '; // Combinator
  ...
  protected function seek ($selector, &$ret, $parent_cmd, $lowercase = false) {
    ...
    if ($parent_cmd === ' ') { // Descendant Combinator
      ...
      $end = (!empty($this->_[ATFP_HDOM_INFO_END])) ? $this->_[ATFP_HDOM_INFO_END] : 0;
      if ($end == 0) {
        $parent = $this->parent;
        while (!isset($parent->_[ATFP_HDOM_INFO_END]) && $parent !== null) {
          $end    -= 1;
          $parent = $parent->parent;
        }
        $end += $parent->_[ATFP_HDOM_INFO_END];
      }

      // Get list of target nodes
      $nodes_start = $this->_[ATFP_HDOM_INFO_BEGIN] + 1;
    ...
    foreach ($nodes as $node) {
      ...
      if ($pass) $ret[$node->_[ATFP_HDOM_INFO_BEGIN]] = 1;
      unset($node);
    }
    // It's passed by reference so this is actually what this function returns.
    if (is_object($atfp_debug_object)) {
      $atfp_debug_object->debug_log(1, 'EXIT - ret: ', $ret);
    }
  }
  ...
  protected function parse_selector ($selector_string) {
    ...
    if (is_object($atfp_debug_object)) {
      $atfp_debug_object->debug_log(2, 'Matches Array: ', $matches);
    }
  ...
  function __set ($name, $value) {
    ...
    switch ($name) {
      case 'outertext':
        return $this->_[ATFP_HDOM_INFO_OUTER] = $value;
      case 'innertext':
        if (isset($this->_[ATFP_HDOM_INFO_TEXT])) {
          return $this->_[ATFP_HDOM_INFO_TEXT] = $value;
        }
        return $this->_[ATFP_HDOM_INFO_INNER] = $value;
    }

    if (!isset($this->attr[$name])) {
      $this->_[ATFP_HDOM_INFO_SPACE][] = [' ', '', ''];
      $this->_[ATFP_HDOM_INFO_QUOTE][] = ATFP_HDOM_QUOTE_DOUBLE;
    }
  ...
  function addClass ($class) {
    ...
    if (is_array($class)) {
      ...
      if (is_object($atfp_debug_object)) {
        $atfp_debug_object->debug_log(2, 'Invalid type: ', gettype($class));
      }
    }
  }
...
class ATFP_simple_html_dom {
  ...
  function load (
    ...
    if ($options & ATFP_HDOM_SMARTY_AS_TEXT) { // Strip Smarty scripts
    ...
    $this->parse();
    // end
    $this->root->_[ATFP_HDOM_INFO_END] = $this->cursor;
    $this->parse_charset();
  ...
  protected function prepare (
    ...
    $this->root                          = new ATFP_simple_html_dom_node($this);
    $this->root->tag                     = 'root';
    $this->root->_[ATFP_HDOM_INFO_BEGIN] = -1;
    $this->root->nodetype                = ATFP_HDOM_TYPE_ROOT;
  ...
  protected function parse () {
    while (true) {
      ...
      $node = new ATFP_simple_html_dom_node($this);
      ++$this->cursor;
      $node->_[ATFP_HDOM_INFO_TEXT] = $s;
  ...
  protected function parse_charset () {
    ...
    if (empty($charset)) {
      ...
      if ($meta = $this->root->find('meta[charset]', 0)) {
        ...
        if (is_object($atfp_debug_object)) {
          $atfp_debug_object->debug_log(2, 'meta charset: ' . $charset);
        }
      }
    }
    ...
    if (empty($charset)) {
      ...
      if (function_exists('mb_detect_encoding')) {
        ...
        if ($encoding !== false) {
          $charset = $encoding;
          if (is_object($atfp_debug_object)) {
            $atfp_debug_object->debug_log(2, 'mb_detect: ' . $charset);
          }
        }
      }
    }
    ...
    if (empty($charset)) {
      ...
      $charset = 'UTF-8';
      if (is_object($atfp_debug_object)) {
        $atfp_debug_object->debug_log(2, 'No match found, assume ' . $charset);
      }
    }
    ...
    if (is_object($atfp_debug_object)) {
      $atfp_debug_object->debug_log(1, 'EXIT - ' . $charset);
    }
  ...
  protected function read_tag () {
    ...
    if ($this->char === '/') {
      ...
      if ($parent_lower !== $tag_lower) {
        ...
        if (
          ...
          $this->parent->_[ATFP_HDOM_INFO_END] = 0;
          ...
          if (strtolower($this->parent->tag) !== $tag_lower) {
            ...
            $this->parent->_[ATFP_HDOM_INFO_END] = $this->cursor;
          ...
          $this->parent->_[ATFP_HDOM_INFO_END] = 0; // No end tag
          $org_parent                          = $this->parent;
    ...
    // start tag
    $node                          = new ATFP_simple_html_dom_node($this);
    $node->_[ATFP_HDOM_INFO_BEGIN] = $this->cursor;
    ++$this->cursor;
    ...
    if (isset($tag[0]) && $tag[0] === '!') {
      $node->_[ATFP_HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until_char('>');

      if (isset($tag[2]) && $tag[1] === '-' && $tag[2] === '-') { // Comment ("<!--")
        $node->nodetype = ATFP_HDOM_TYPE_COMMENT;
        $node->tag      = 'comment';
      } else { // Could be doctype or CDATA but we don't care
        $node->nodetype = ATFP_HDOM_TYPE_UNKNOWN;
        $node->tag      = 'unknown';
      }

      if ($this->char === '>') {
        $node->_[ATFP_HDOM_INFO_TEXT] .= '>';
      }
    ...
    if ($pos = strpos($tag, '<') !== false) {
      $tag                          = '<' . substr($tag, 0, -1);
      $node->_[ATFP_HDOM_INFO_TEXT] = $tag;
      $this->link_nodes($node, false);
    ...
    if (!preg_match('/^\w[\w:-]*$/', $tag)) {
      $node->_[ATFP_HDOM_INFO_TEXT] = '<' . $tag . $this->copy_until('<>');
    ...
    $node->nodetype = ATFP_HDOM_TYPE_ELEMENT;
    $tag_lower      = strtolower($tag);
    ...
    do {
      ...
      if ($this->pos >= $this->size - 1 && $this->char !== '>') {
        $node->nodetype               = ATFP_HDOM_TYPE_TEXT;
        $node->_[ATFP_HDOM_INFO_END]  = 0;
        $node->_[ATFP_HDOM_INFO_TEXT] = '<' . $tag . $space[0] . $name;
        $node->tag                    = 'text';
        $this->link_nodes($node, false);
        return true;
      }
      ...
      if ($this->doc[$this->pos - 1] == '<') {
        ...
        $node->attr                   = [];
        $node->_[ATFP_HDOM_INFO_END]  = 0;
        $node->_[ATFP_HDOM_INFO_TEXT] = substr(
          $this->doc,
          $begin_tag_pos,
      ...
      if ($name !== '/' && $name !== '') { // this is a attribute name
        ...
        if ($this->char === '=') {                                                    // attribute with value
          ...
          //no value attr: nowrap, checked selected...
          $node->_[ATFP_HDOM_INFO_QUOTE][] = ATFP_HDOM_QUOTE_NO;
          $node->attr[$name]               = true;
          if ($this->char != '>') {
        ...
        $node->_[ATFP_HDOM_INFO_SPACE][] = $space;
    ...
    $this->link_nodes($node, true);
    $node->_[ATFP_HDOM_INFO_ENDSPACE] = $space[0];

    // handle empty tags (i.e. "<div/>")
    if ($this->copy_until_char('>') === '/') {
      $node->_[ATFP_HDOM_INFO_ENDSPACE] .= '/';
      $node->_[ATFP_HDOM_INFO_END]      = 0;
    } else {
      // reset parent
    ...
    if ($node->tag === 'br') {
      $node->_[ATFP_HDOM_INFO_INNER] = $this->default_br_text;
    }
  ...
  protected function parse_attr ($node, $name, &$space) {
    ...
    switch ($this->char) {
      case '"':
        $quote_type = ATFP_HDOM_QUOTE_DOUBLE;
        $this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
        $value      = $this->copy_until_char('"');
        $this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
        break;
      case '\'':
        $quote_type = ATFP_HDOM_QUOTE_SINGLE;
        $this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
        $value      = $this->copy_until_char('\'');
        $this->char = (++$this->pos < $this->size) ? $this->doc[$this->pos] : null; // next
        break;
      default:
        $quote_type = ATFP_HDOM_QUOTE_NO;
    ...
    if (!$is_duplicate) {
      $node->_[ATFP_HDOM_INFO_QUOTE][] = $quote_type;
  ...
  protected function remove_noise ($pattern, $remove_tag = false) {
    ...
    for ($i = $count - 1; $i > -1; --$i) {
      ...
      if (is_object($atfp_debug_object)) {
        $atfp_debug_object->debug_log(2, 'key is: ' . $key);
      }
  ...
  function restore_noise ($text) {
    ...
    while (($pos = strpos($text, '___noise___')) !== false) {
      ...
      if (strlen($text) > $pos + 15) {
        ...
        if (is_object($atfp_debug_object)) {
          $atfp_debug_object->debug_log(2, 'located key of: ' . $key);
        }
  ...
  function createElement ($name, $value = null) {
    return @ATFP_str_get_html("<$name>$value</$name>")->firstChild();
  }

  function createTextNode ($value) {
    return @end(ATFP_str_get_html($value)->nodes);
  }
